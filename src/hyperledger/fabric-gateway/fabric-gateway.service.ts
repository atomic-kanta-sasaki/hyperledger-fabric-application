...
export class FabricGatewayService  {
    ...
    async createConnection() {
        const grpcConnection = await this.clientProvider.newGrpcConnection('localhost:7051', 'peer0.org1.example.com', 'ca.crt');
        const identity = await this.identityProvider.newIdentity('Org1MSP','cert.pem');
        const signer = await this.signerProvider.newSigner('private_key.pem')
